<template lang="pug">
v-footer(padless)
  v-container.mt-8.ma-0.pa-0(fluid)
    v-card.ma-0.pa-0.blue-grey.darken-4.white--text.text-center(flat tile)
      v-card-text
        v-btn.mx-4.white--text.elevation-1(v-for='media in medias' :key='media.link' icon @click="openUrl(media)")
          v-icon(size='24px') {{ media.icon }}
